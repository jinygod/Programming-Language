...
SELECT ename, sal, to_char(sal, '999,999') FROM emp;        -- 천 단위로 콤마
...
SELECT ename, sal, to_char(sal, '000,000.00') FROM emp;
SELECT ename, sal, to_char(sal, '9,000') FROM emp; 
SELECT ename, sal, to_char(sal, '9,990') FROM emp; 
...
SELECT to_char(trunc(1234.98), '999,999') FROM dual;    -- 반올림하지 않음(1,234)

-- 화폐단위
-- L:Locale
SELECT sal, to_char(sal * 10000, 'L999,999,999') FROM emp;

SELECT sal, to_char(sal * 10, '$999,999,999') FROM emp;
