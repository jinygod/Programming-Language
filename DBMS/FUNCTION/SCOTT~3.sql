...
-- date + n(달수)
-- 현재날짜에서 6개월 후의 날짜
SELECT sysdate, add_months(sysdate, 6) FROM dual; -- 한 달 뒤
...
SELECT last_day('2022/02/01') "2월의 마지막 날짜" FROM dual;

-- 이 달의 남은 일수
SELECT trunc(last_day('2022/05/01') - sysdate  + 1) from dual ;

-- 올해의 남은 일수, 남은 개월 수 
select trunc((last_day('2022/12/01') - sysdate) + 1) from dual;
select trunc(((last_day('2022/12/01') - sysdate) + 1) / 30.5) from dual;