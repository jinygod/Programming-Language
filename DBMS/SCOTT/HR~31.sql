...
SELECT salary, commission_pct,
              ...
              salary * 12 + (salary * commission_pct * 12) as "총연봉"
FROM employees
WHERE commission_pct IS NOT NULL ;    -- 전체 검색조건에서 제외

-- 모든 사원의 연봉과 수당을 포함한 총 연봉을 계산
-- 수당(commision_pct) 컬럼이 널인 경우 ZERO으로 처리
-- NVL(컬럼, 값) : 대상 컬럼이 널인 경우 지정된 값을 대체 처리
SELECT salary, nvl(commission_pct, 0),
              salary * nvl(commission_pct, 0)* 12 as "연수당",
              salary * 12 + (salary * nvl(commission_pct, 0 ) * 12) as "총연봉"
FROM employees;

-- 기존의 검색 결과로 새로운 가상의 테이블 생성
CREATE TABLE employee
AS
SELECT employee_id as ENO, first_name || ', ' || last_name as ENAME,
