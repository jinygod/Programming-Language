...
-- 사원('JAMES')의 부서코드와 동일한 부서에서 근부하는 사원은?
SELECT * FROM emp
...
SELECT * FROM emp WHERE sal < 2975; -- 2975를 모른다
SELECT * FROM emp WHERE sal < (SELECT sal FROM emp WHERE empno = 7566);

-- 서브쿼리의 결과가 다행행이 되면 안된다.
...
SELECT ename, hiredate FROM emp WHERE ename = 'TURNER';
...
-- [문제2]
-- 'TURNER'의 입사일자보다 늦게 입사한 사원(TURENER 포함)
SELECT ename, hiredate 
    FROM emp 
    WHERE hiredate >= (SELECT hiredate FROM emp WHERE ename = 'TURNER')
    ORDER BY hiredate;
...
-- [문제4]
-- 부서코드가 10, 20에 속한 사원중에서 전체 사원의 평균 급여보다 높은 급여를 받는
-- 사원 정보와 소속 부서 정보
SELECT avg(sal) FROM emp;   -- 평균급여:2225
SELECT * 
    FROM emp e, dept d
    WHERE e.deptno IN(10, 20) AND e.deptno = d.deptno
