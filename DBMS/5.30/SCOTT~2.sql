...
-- 부서코드가 30인 부서를 제외한
-- 부서별 평균 급여액이 2500미만인 부서
-- 1단계: 30번 부서를 제외
-- 2단계: 부서별로 평균 급여액을 집계
-- 3단계: 평균 급여액이 2500미만인 부서만 선택
-- 4단계: 부서번호를 기준으로 오름차순으로 정렬(sort)
SELECT deptno,
    avg(sal)
    FROM emp
    WHERE deptno <> 30         -- 1단계: 30번 부서를 제외
    GROUP BY deptno            -- 2단계: 부서별로 평균 급여액을 집계
    HAVING avg(sal) < 2500    -- 3단계 : 평균 급여액이 2500미만인 부서만 선택
    ORDER BY deptno;            -- 4단계: 부서번호를 기준으로 오름차순으로 정렬(sort)
    
-- 집계된 데이터를 기준으로 정렬
-- job이 MANAGER를 제외하고
-- 총 급여액이 5000 이상인 JOB 별 급여 총액
