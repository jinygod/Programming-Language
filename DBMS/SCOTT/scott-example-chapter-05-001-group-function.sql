...
-- 입사일자: 최근 입사일, 가장 오래된 입사일
SELECT max(hiredate),   -- 가장 최근 입사자의 입사일
    min(hiredate)       -- 가장 오래된 입사자 입사일
    FROM emp;
    
-- NULL은 그룹함수 대상에서 제외    
SELECT count(*),    -- 전제 행의 갯수
    count(comm)     -- 컬럼(comm)에서 값이 있는 행의 갯수
    FROM emp; 
...
SELECT count(sal),          -- 급여를 받는 사원의 총수
    count(DISTINCT sal)     -- 급여를 받는 사원의 총수(동일 급여를 받는 사원의 중복을 제거)
    FROM emp;
