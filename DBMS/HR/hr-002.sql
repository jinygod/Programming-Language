...
-- 입력: INSERT
-- 테이블에 자료 입력
INSERT INTO hello (id, name, tel, age) VALUES (1234, '홍길동', '010-0007-7777', 31);
...
UPDATE hello            -- 테이블 이름
    SET id = 1000       -- 컬럼=값
    WHERE id = 1234;    -- 조건:변경한 데이터 
    
-- 여러 컬럼의 내용을 변경할 때는 콤마로 분리
...
-- 검색조건: 변경된 대상을 지정하지 않으면 모든 데이터가 변경 된다.
UPDATE hello SET 
    name = '고길동',
    tel = '123-1234-1234',
    age = 39;
...
-- [중요]
-- DELETE, UPDATE를 할 때 WHERE 조건을 넣지 않으면
-- 모든 데이터가 영향을 받는다.
