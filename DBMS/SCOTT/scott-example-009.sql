...
-- 널처리를 하지 않으면 총급가 null로 나옴
SELECT ename, sal, comm,    
    sal * 12 + comm "총급여"
    FROM emp
    ORDER BY job;
    
-- 인센티브(comm)가 널인 경우 0으로 계산    
SELECT ename, sal, comm,
...
-- 여러 컬럼중에서 널이 아닌 값을 순서대로 찾을 때 유용
SELECT ename, sal, comm,
    coalesce(comm, sal, 0)
    FROM emp;
...
SELECT ename, sal, job,
    decode(job,
        'MANAGER', sal * 0.9,
        'SALEMAN', sal * 0.5,
        'ANALYST', sal * 0.3, sal * 0.2) "특별수당"
    FROM emp;

-- CASE
-- CASE 다음에 컬럼명이 나오면 
-- 컬럼의 해당하는 값이 WHEN에 일치되는 값을 선택
SELECT ename, sal, job,
    CASE job
        WHEN 'MANAGER' THEN sal * 0.9
        WHEN 'SALEMAN' THEN sal * 0.5
        WHEN 'ANALYST' THEN sal * 0.3
        ELSE sal * 0.2
    END as "특별수당"
    FROM emp;

-- CASE 다음에 컬럼명이 나오지 않으면 WHEN에 조건식이 나와야 한다.
SELECT ename, sal, job,
    CASE
        WHEN job = 'MANAGER' THEN sal * 0.9
        WHEN job = 'SALEMAN' THEN sal * 0.5
