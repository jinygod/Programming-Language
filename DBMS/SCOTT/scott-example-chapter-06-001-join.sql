...
INSERT INTO emp VALUES (8100, 'DAVID', 'ALALYST', 7839, '1991/10/01', 4000, 400, 50);
COMMIT;

-- 카디시안 곱(Cartesian Product) : 크로스 조인, 교차 조인
-- 조건없이 조인을 하면 조인되는 테이블의 행의 갯수의 곱
-- 부서와 사원 조인 : 5 * 13 = 65
SELECT count(*) FROM dept, emp;
SELECT * FROM dept, emp;

-- 부서(50) : 'SEOUL KOREA' -> 'SEONGNAM GYEONGGI'
UPDATE dept 
    SET loc = 'SEONGNAM'
    WHERE deptno = 50;
COMMIT;    
...
-- 사원정보(emp), 부서정보(dept)
-- 정렬(사번)
SELECT e.empno, e.ename, e.job, e.deptno, e.sal, e.comm, d.deptno, d.dname, d.loc
    FROM emp e, dept d 
